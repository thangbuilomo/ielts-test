(function () {
  ...
  const els = {
    ...
    studentRealEmail: document.getElementById('studentRealEmail'),
  ...
  function activateExam(options) {
    ...
    state.realEmail = options.realEmail || '';
  ...
  async function startTest() {
    ...
    const fullName = els.studentFullName.value.trim();
    const phone = els.studentPhone.value.trim();
    const realEmail = els.studentRealEmail.value.trim();
    els.startError.textContent = '';

    if (!email || !password.trim()) {
      els.startError.textContent = 'Vui lòng kiểm tra lại tài khoản và mật khẩu.';
      els.studentEmail.focus();
      return;
    }
    if (!fullName || !phone) {
      els.startError.textContent = 'Vui lòng nhập đầy đủ Họ Tên và Số điện thoại.';
      return;
    }
  ...
  function showReceipt(payload, sent) {
    els.receiptContent.innerHTML = `
      <table class="receipt-table">
        ...
        <tr><td>Email cá nhân</td><td>${escapeHtml(payload.real_email || '')}</td></tr>
